...
const CartPage = () => {
    ...
    const [ cart, cartSet ] = useState( localStoreService.getLocal('CartBuy') );
    ...
    const clickStep = ( op, step, op1, op2, op3) => {
        ...
        if (op === 'plus') {
            ...
            //     if ( item.id === op1 && item.color === op2 && item.size === op3  ) {
            //
            //         cart[index].step = +step + 1;
            //         console.log('op', index, cart[index] );
            //
            //         cartSet(cart);
            //         localStoreService.saveLocal('CartBuy', cart );
            //     }
        ...
        if (op === 'min') {
            ...
            // console.log('min >>', step, op, op1, op2, op3, cart)
                                                        ...
                                                        <div className="wrapStep">
                                                            <div className="row">
                                                                <div className="col">
                                                                    ...
                                                                    <div className="step">
                                                                        <span onClick={()=>clickStep('min', item.step, item.id, item.color, item.size)} className="min">-</span>
                                                                        <span>{item.step}</span>
                                                                        <span onClick={()=>clickStep('plus', item.step, item.id, item.color, item.size)} className="plus">+</span>
                                                                    </div>
                                                                </div>
